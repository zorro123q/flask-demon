...
import os
# 确保 SparkApi.py 文件在同一个目录中，或者调整路径以正确导入
from SparkApi import spmain
...
@app.route('/anay')
def anay_view():
    return render_template('anay.html')
...
def service():
    ...
    # 假设这里的spmain是您处理问题的函数
    token1 = spmain(appid="27e1d833", api_key="8821d6c48c4c9b1ec79c8ac05b2be38b",
                    api_secret="NDQ1MDgzNTk5NDQyZjgwMjAwNjhiYTli", gpt_url="wss://spark-api.xf-yun.com/v2.1/chat",
                    question=question)  # 使用获取的question变量
    token = ''.join(token1)

    # 使用render_template渲染页面，并传递token变量
    return render_template('service.html', token=token)
